...
description: >
  ...
  review architecture with Codex model-steered agents. Use this skill when the user asks to review
...
Use explicit model steering. Do **not** let every subagent inherit the main
session model, and do **not** default the whole workflow to GPT-5.5. Reserve the
most expensive model only for escalations where it will change the outcome.

| Step | Agent | Preferred Codex model | Reasoning |
|------|-------|-----------------------|-----------|
| Step 0 | Scope, problem reconstruction, and context gathering | `gpt-5.4-mini` | `medium` |
| Phase 1a | AI Authorship Detection | `gpt-5.4` | `high` |
| Phase 1b | Idiom Fluency | `gpt-5.4` | `high` |
| Phase 1c | Code Quality | `gpt-5.3-codex` | `medium` |
| Phase 1d | Architecture and Solution-Fit | `gpt-5.4` | `high` |
| Phase 2 | Calibration | `gpt-5.4` | `high` |
| Phase 3 | Synthesis | Inline in the main session | n/a |

Escalate a specific step to `gpt-5.5` with `high` or `xhigh` reasoning only when
one of these is true:

- Step 0's gathered context bundle exceeds roughly 150k tokens after trimming.
- Phase 1d must evaluate a cross-system architectural decision with unclear
  ownership, reviewer disagreement, or major operational consequences.
- Phase 2 has high-confidence but conflicting lens findings and the final verdict
  affects whether the PR should merge.
- The user explicitly asks for maximum-depth review.

If a preferred model is unavailable, choose the nearest cheaper/faster model with
the same reasoning tier before inheriting the main session model. If fallback to
inheritance is unavoidable, state that in the review notes.

### Context window

Keep every step on a bounded context diet. Trim context before escalating model
size. If you must escalate, only escalate the *specific* step that needs it
(usually Phase 2 calibration, which sees the union of all Phase 1 findings) —
not every subagent.
...
Launch a Step 0 Codex subagent with `model: "gpt-5.4-mini"` and
`reasoning_effort: "medium"` when subagents are available.
...
3. Detect the primary language(s) and load the appropriate reference file(s) from
   `references/` beside this skill -- only read reference files
...
fabricate acceptances; do not infer them from project guidance docs.
...
**Important:** Use Codex's default subagent type (or a read-only explorer when the runtime
offers one and the lens only needs inspection). Do NOT use specialized review agents or
other review plugins (coderabbit, feature-dev, pr-review-toolkit, etc.) -- this skill
provides its own complete review methodology, and specialized agents will blend their own
prompts with these instructions, producing inconsistent results.
...
overestimating only adds one extra deep-reasoning pass. Quick generated-file tells: a header
...
#### Phase 1a: AI Authorship Detection (`gpt-5.4`, high reasoning)
...
#### Phase 1b: Idiom Fluency (`gpt-5.4`, high reasoning)
...
#### Phase 1c: Code Quality (`gpt-5.3-codex`, medium reasoning)
...
#### Phase 1d: Architecture and Solution-Fit Review (`gpt-5.4`, high reasoning)
...
### Phase 2: Calibration review (`gpt-5.4`, high reasoning)

Launch a **separate, independent** Codex subagent with `model: "gpt-5.4"` and
`reasoning_effort: "high"` unless the escalation rules above require `gpt-5.5`.
This agent receives ALL findings from all Phase 1 lenses, the original files, the
problem reconstruction, reviewer comments, the codebase context, and the idiom baseline.
...
Language-specific signals live in `references/` beside this skill.
...
**Cost optimization -- reference file discipline.** These reference files are static
between runs against the same codebase. Load only the language references that match the
review scope, and pass only the relevant excerpts to each lens. Do not broadcast every
reference file to every subagent.
...
- **Interactive** — a human is in the loop (Codex session or IDE extension).
  Ask the user directly when a delivery choice is genuinely needed.
- **Non-interactive** — running headless in CI/CD (GitHub Actions, scheduled
  job, automation). Do not wait for user input; choose the documented default.
...
- **Skip interactive user-input tools entirely.** Never wait for a human
  response in CI.
...
In interactive mode, ask the user how to surface the findings using the
runtime's user-input primitive when available, or a concise plain-text
question otherwise. The exact options depend on whether a PR was detected:
...
If the runtime supports structured choices, include an **"Other"** escape
hatch when available and mark the PR option as `(Recommended)` when a PR is
detected.
...
ask in their next turn. Do not present a 1-option menu; a single-choice ask
is friction without information.
...
<sub>Generated by `/code-quality:slop` · 4-lens parallel scan + Codex calibration</sub>
```
...
`SLOP_REVIEW.md` at the repo root, commit, and push. Tell the user
