...
Arc's workflow is a chain of skills and agents: brainstorm → plan → build → review/evaluate → verify → finish. Each link has specific behavioral contracts — the build skill must never write code directly, the evaluator must never read the diff, the reviewer must re-review after fixes. These contracts are easy to violate subtly (e.g., a model "helpfully" writing a quick fix instead of dispatching a subagent). Evals catch these violations before they compound into broken workflows.
...
evals/
  triggers/                    # Does the right thing activate?
    skills.json                # Workflow skills (brainstorm, plan, build, etc.)
    commands.json              # Slash commands (/arc:ready, /arc:create, etc.)
    agents.json                # Agent dispatch (issue-manager, builder, code-reviewer, evaluator, etc.)
  quality/                     # Once activated, does it behave correctly?
    ...
    build.json                 # Build skill output (16 evals)
    debug.json                 # Debug skill output (5 evals)
    verify.json                # Verify skill output (5 evals)
    evaluate.json              # Evaluator agent output (13 evals)
...
|---|---|---|
| Eval 1 | Information asymmetry | Evaluator read the diff or builder's tests — independence compromised |
...
| Eval 12 | Broken build → FAIL | Evaluator reported BLOCKED when the builder's build is broken — should be FAIL |
| Eval 13 | Evaluator setup failure → BLOCKED | Evaluator reported FAIL when its own tests didn't compile — should be BLOCKED |

The `build.json` evals 10-16 test the orchestrator's integration with the evaluator:
- Eval 10: Handles evaluator FAIL by re-dispatching builder
...
- Eval 14: Handles evaluator BLOCKED — does not blame builder
- Eval 15: Handles evaluator FAIL due to broken builder build
...
- A skill's workflow changes (e.g., build now dispatches evaluator + reviewer in parallel)
- New behaviors are added (e.g., evaluator circuit breaker in build)
