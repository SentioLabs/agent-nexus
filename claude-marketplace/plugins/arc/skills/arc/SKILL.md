...
- `arc plan` - Register and review design plans (create, show, approve, comments)
...
Design docs live in `docs/plans/` as filesystem markdown. Arc registers them on the **planner** review surface, at create time by the `/arc:brainstorm` skill. `/arc:plan` and any other consumer reads line 1 of the doc — `<!-- arc-review: id=<id> -->` — to get the plan ID.

**Surface:**

| Create command | URL pattern | Best for |
|---|---|---|
| `arc plan create <file>` | `http://localhost:7432/planner/<id>` | Solo review, plain HTTP, a comment thread on a markdown render |

The planner is plain HTTP and local-only — the reviewer's browser must reach `localhost:7432` on this machine. There are no edit tokens; the URL is just the planner path.

### `arc plan` commands

| Command | Purpose |
|---------|---------|
| `arc plan create <file-path>` | Register a plan on the `/planner/<id>` surface (plain HTTP, no encryption). There's no in-place update — re-running `create` produces a new ID. |
...
create → reviewers leave comments → author replies inline → `arc plan comments` surfaces the thread to the implementation flow. Approved design content is written into the epic's description field when creating implementation tasks. Run `arc docs plans` for full details.

The `<!-- arc-review: id=… -->` marker on line 1 of every registered design doc tells downstream skills the plan ID. See `skills/brainstorm/SKILL.md` step 6 for the marker-write contract and `skills/plan/SKILL.md` step 1 for the read pattern.
