...
- **Use explicit Codex model steering when running as a dedicated review agent
  or automation.** Default ordinary size reviews to `gpt-5.4-mini` with
  `reasoning_effort: "medium"`.
- **Escalate only the judgment-heavy cases.** Use `gpt-5.3-codex` with
  `reasoning_effort: "high"` when the threshold check fires because the PR spans
  3+ top-level directories, has multiple plausible seams, or requires reading
  source content to verify a seam. Use `gpt-5.4` with `high` only for difficult
  cross-system split decisions. Do not use GPT-5.5 for size-review unless the
  user explicitly asks for maximum-depth review.
- **Keep context bounded.** Larger-context models are wasted capacity for shape
  analysis; do not escalate unless a specific seam check requires reading large
...
For high-volume CI usage (every PR), run this skill on `gpt-5.4-mini` with medium
reasoning by default and reserve the higher tiers for PRs flagged by the thresholds
above.
...
1. **Bundled universal defaults** — read
   `references/default-exclusions.md` beside this skill.
...
After producing the report, ask the user how to surface it:
...
If a stack plan was produced and the user wants to act on it, offer to use an
available stacking workflow (for example git-spice, Graphite, Sapling, or plain
git branches) and translate the plan into concrete commands for that tool.
