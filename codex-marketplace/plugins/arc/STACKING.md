...
How to use the [`arc`](./skills/arc/SKILL.md) plugin with `git-spice` when an
...
doc just describes the integration points and the order of operations. Read
`arc/skills/arc/SKILL.md` and your local `git-spice` docs for the underlying
mental models.
...
# Use the guidance and prompt template from skills/build/SKILL.md.
...
| If you're … | Dispatch |
|---|---|
| Implementing one arc task end-to-end (TDD + tests) | `builder` (per arc:build's loop) |
| Building a stack of N branches in one uninterrupted pass with no per-task review gates | Use your local git-spice stacker workflow, if installed |
| Stacking arc tasks AND keeping arc's review gates per task | **No subagent**. The orchestrator runs the loop in step 3 above. Do not dispatch a generic stacker; it does not know about arc's spec-review / code-review / evaluator gates. |
| Recovering a wedged stack (rebase failed, branches diverged) | Use your local git-spice recovery workflow |
| Auditing an arc spec / code review | `spec-reviewer`, `code-reviewer`, `evaluator` (unchanged) |
...
For wedged-stack recovery beyond the above, use your local git-spice recovery workflow.
...
- **Cover non-arc git-spice usage.** For those, follow your local git-spice docs.
